...
const Tasks = () => {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const updatedTasks = tasks.map(task => storedCompletedTasks.includes(task.id) ? { ...task, completed: true } : task);
        setTasks(updatedTasks);
    }, []);
    const [selectedTask, setSelectedTask] = (0, react_1.useState)(null);
    const [validationAttempt, setValidationAttempt] = (0, react_1.useState)(0);
    const [selectedSection, setSelectedSection] = (0, react_1.useState)('main');
    // const handleValidateClick = () => {
    //   if (validationAttempt === 0) {
    //     alert("Error while validating. Make sure to perform your task before validating.");
    //     setValidationAttempt(1);
    ...
    const handleValidateClick = () => {
        if (validationAttempt === 0) {
            ...
            const updatedTasks = tasks.map(task => task.id === selectedTask.id ? { ...task, completed: true } : task);
    ...
    return (<div className="task-container" style={{ width: '100%' }}>
      <link_1.default href="/"><img src="/images/info/output-onlinepngtools (6).png" width={24} height={24} alt="back"/></link_1.default>
      ...
      <div className="tasks-display">
        {filteredTasks.map((task) => (<div key={task.id} className="task-containers" style={{ width: '100%' }}>
      ...
      {selectedTask && (<div className="task-popup">
    ...
    <div className="tasks-display">
      ...
      {completedTasks.map(task => (<div key={task.id} className="task-containers" style={{ width: '100%' }}>
