...
bot.start(async (ctx) => {
    try {
        ...
        console.log(`Payload: ${startPayload || 'None'}`); 
        ...
        try {
            ...
            const userResultText = await userResponse.text();

            if (userResponse.ok || userResponse.status === 409) {
                userCreatedOrFound = true;
                console.log(`✅ User ${userId} active in DB (Status: ${userResponse.status})`);
            } else {
                console.error(`❌ User API Error: ${userResponse.status} - ${userResultText}`);
            }
        } catch (error) {
            console.error('❌ Network error in User API:', error);
        }
        ...
        if (referrerId && referrerId !== '' && referrerId !== 'signup_smartsnail_pass') {
            console.log(`🔗 Referral detected! Referrer: ${referrerId}`);
            
            try {
                if (!/^\d+$/.test(referrerId)) {
                    console.log(`⚠️ Referral skipped: Invalid format (Not a number)`);
                } else if (referrerId === userId.toString()) {
                    console.log(`⚠️ Referral skipped: User tried to refer themselves`);
                } else {
                    console.log(`⏳ Sleeping 2s to allow DB indexing...`);
                    ...
                    if (referralResponse.ok && referralData.success) {
                        ...
                        console.log(`ℹ️ Referral not applied: ${referralData.error || referralData.existing ? 'User already referred' : 'Unknown'}`);
                    }
                }
            } catch (error) {
                console.error('❌ Referral Logic Error:', error);
            }
        }

        // Step 3: Handle Pass Registration
        if (startPayload === 'signup_smartsnail_pass') {
            console.log(`🐌 Handling SmartSnail Pass Flow`);
            ctx.session = { step: 'NAME' };
            return ctx.reply("🐌 **SMARTSNAIL PASS REGISTRATION**\n\nWhat's your name?");
        }

        // Step 4: Final Welcome Message
            ...
            reply_markup: {
                inline_keyboard: [
                    [{
                        ...
                        web_app: { url: "https://telegram-smartsnail-airdrop.vercel.app" },
                    }],
                ],
            },
        });
        ...
        console.error('🔥 GLOBAL CRASH IN START HANDLER:', error);
    }
});
...
bot.action(/set_sched_(.+)/, async (ctx) => {
  ...
  if (ctx.from.id.toString() !== adminId) {
    return ctx.answerCbQuery("❌ Only Sage Admin can do this.", { show_alert: true });
  }
