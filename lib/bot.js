    ...
    try {
      ...
      await prisma.user.update({
        ...
        data: {
          ...
          // Un-comment these once you've updated your Prisma schema!
          // homeCountry: ctx.session.country,
          // homeCityState: normalizedCityState
        }
      });

      // 2. SEARCH FOR PARTNERS (With Case-Insensitive Mode)
      const partners = await prisma.partner.findMany({
        where: {
          country: {
            equals: ctx.session.country,
            mode: 'insensitive' // <--- CRITICAL FIX
          },
          cityState: {
            equals: normalizedCityState,
            mode: 'insensitive' // <--- CRITICAL FIX
          }
        }
      });
      ...
      if (partners.length === 0) {
        ctx.session.step = null; // Clear session even if no gym found
        return ctx.reply(`Pass Registered! 🐌\n\nNo partners in ${normalizedCityState} yet, but you're ready for global entry.`);
      }

      const buttons = partners.map(p => [
        // Ensure p.id is converted to a string for the URL
        Markup.button.url(`🏢 ${p.name}`, `https://t.me/SmartSnailBot/app?startapp=gymId_${p.id.toString()}`)
      ]);

      ctx.reply("Pass Activated! 🎟️\nHere are the partners in your area:", Markup.inlineKeyboard(buttons));
