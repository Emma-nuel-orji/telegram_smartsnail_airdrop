...
bot.action(/set_sched_(.+)/, async (ctx) => {
  if (ctx.from.id.toString() !== process.env.NEXT_PUBLIC_ADMIN_TELEGRAM_ID) {
  ...
  await ctx.reply("📅 **Setting Combat Schedule**\n\nWhich days will they train? Send numbers:\n1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat, 0=Sun\n\nExample: `1,3,5` (for Mon, Wed, Fri)", { parse_mode: 'Markdown' });
...
bot.on('text', async (ctx) => {
  ...
  // ==== SAGE COMBAT SCHEDULE INITIALIZATION ====
const SAGE_ADMIN_ID = process.env.NEXT_PUBLIC_ADMIN_TELEGRAM_ID;

if (ctx.session?.step === 'awaiting_combat_days') {
  // Security Check: Only the Sage Admin ID from your .env can do this
  if (ctx.from.id.toString() !== SAGE_ADMIN_ID) {
    return ctx.reply('❌ Access Denied: Only the Sage Combat Lead can set training days.');
  }

  const subId = ctx.session.targetSubId;
  
  // Clean the input: turn "1, 3, 5" into [1, 3, 5]
  ...
  try {
    // Update the subscription with the chosen days
    await prisma.subscription.update({
      where: { id: subId },
      data: { 
        trainingDays: days,
        // Optional: you can add a field like 'isInitialized: true' if needed
      }
    });

    const dayNames = days.map(d => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ');
    
    await ctx.reply(`✅ **Schedule Set!**\n\nTraining Days: ${dayNames}\n\nThe user's Snail Pass has been updated with the 31-day training grid.`, { parse_mode: 'Markdown' });
    
    // Clear session to prevent accidental triggers
    ctx.session.step = null;
    ctx.session.targetSubId = null;
  } catch (error) {
    console.error('Error updating combat schedule:', error);
    ctx.reply('❌ Database error. Could not save schedule.');
  }
  return;
}
