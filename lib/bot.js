...
bot.on("pre_checkout_query", async (ctx) => {
  try {
    ...
    // HANDLE TICKETS (Your existing code)
    if (payload.type === "ticket_purchase") {
      await ctx.answerPreCheckoutQuery(true);
      return;
    }

    // HANDLE BOOKS (The new logic)
    if (payload.t) {
      ...
      if (pending) {
        ...
        await ctx.answerPreCheckoutQuery(false, "Transaction record not found.");
      }
      return;
    }

    await ctx.answerPreCheckoutQuery(false, "Unknown purchase type.");
  } catch (error) {
...
bot.on("message:successful_payment", async (ctx) => {
  try {
    ...
    // 1. HANDLE TICKETS (Existing)
    if (payload.type === "ticket_purchase") {
       // ... your ticket code ...
       return;
    }

    // 2. HANDLE BOOKS (New - This activates the boost!)
    if (payload.t) {
      const pendingId = payload.t;

      // Wrap everything in a transaction to ensure it's "All or Nothing"
      await prisma.$transaction(async (tx) => {
        ...
        // Prepare the book info for your update function
        const booksToPurchase = [
          ...(pending.fxckedUpBagsQty > 0 ? [{ id: "ID_FOR_BAGS", qty: pending.fxckedUpBagsQty, title: "FxckedUpBags" }] : []),
          ...(pending.humanRelationsQty > 0 ? [{ id: "ID_FOR_HUMAN", qty: pending.humanRelationsQty, title: "Human Relations" }] : [])
        ];
        ...
        await updateDatabaseTransaction(
          tx,
          booksToPurchase,
          [], // Codes if you have them, or handle inside
          pending.telegramId,
          pending.email,
          "STARS",
          pending.amount,
          pending.tappingRate || 0,
          pending.totalPoints || 0,
          pending.orderId
        );

        // Mark as completed so they can't double-claim
      ...
      await ctx.reply("🚀 **Boost Activated!** Your snail is now faster and your rewards have been added.");
    }
  } catch (error) {
...
bot.start(async (ctx) => {
    try {
        ...
        // Step 2: Process referral — no retries needed, API handles pending state
let referralProcessed = false;

if (startPayload === 'signup_smartsnail_pass') {
  ctx.session = { step: 'NAME' };
  return ctx.reply("🐌 **SMARTSNAIL PASS REGISTRATION**\n\nWhat's your name?");
}
...
if (startPayload && startPayload.trim() !== '') {
  ...
  if (!/^\d+$/.test(referrerId)) {
    ...
    try {
      const referralRes = await fetch(`${API_BASE_URL}/api/referrals`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (referralData.success) {
        console.log(`✅ Referral created for user ${userId}`);
        await ctx.reply(`🎉 You joined with a referral! Your referrer has been rewarded.`);
        referralProcessed = true;
      } else if (referralData.pending) {
        // ✅ Will be resolved automatically when user record commits
        console.log(`⏳ Referral pending for user ${userId}`);
        await ctx.reply(`🎉 You joined with a referral! Reward is being processed.`);
        referralProcessed = true;
      } else if (referralData.existing) {
        console.log(`User ${userId} already has a referral`);
        referralProcessed = true;
      } else {
        console.error(`Referral failed:`, referralData);
        await ctx.reply('⚠️ Unable to process referral, but welcome to SmartSnail!');
      }
    } catch (error) {
...
bot.on("message:successful_payment", async (ctx) => {
  try {
    ...
    if (payload.type === "COMBAT_SUBSCRIPTION") {
      const { telegramId, serviceId, planTitle, duration } = payload;
      
      const startDate = new Date();
      const endDate = new Date();
      const d = duration.toLowerCase(); // Consistent defensive check

      // SYNCED LOGIC: Exactly like your Shells API
      if (d.includes("year")) {
      ...
      await prisma.subscription.create({
        data: {
          ...
          // If you have partnerId in your payload, connect it here too!
          planTitle: planTitle,
          status: "ACTIVE",
          startDate,
          endDate,
      ...
      await ctx.reply(`🔥 *Subscription Activated!*\nYour ${planTitle} is now live until ${endDate.toLocaleDateString()}.`, { parse_mode: "Markdown" });
      console.log(`✅ Stars Sub created for ${telegramId}`);
      return;
    }
    // Check if this is an NFT Purchase
    if (payload.type === "NFT_PURCHASE") {
      const { nftId, telegramId } = payload;
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.nft.update({
          where: { id: nftId },
          data: { isSold: true, ownerId: user.id }
        });
      });
      console.log(`✅ NFT ${nftId} successfully assigned to ${telegramId}`);
      return; // Exit here
    }
...
bot.on('message', async (ctx) => {
  ...
  if (step === 'NAME') {
    ...
    ctx.session.nickname = text; 

    ctx.session.step = 'ASK_COUNTRY';

    return ctx.reply("🌍 Which Country are you training in?");

  }

  if (step === 'ASK_COUNTRY') {

    ctx.session.country = text.trim();

    ctx.session.step = 'ASK_CITY_STATE';

    return ctx.reply("📍 Which City/State? (e.g., Lilburn, GA)");

  }

  if (step === 'ASK_CITY_STATE') {

    const normalizedCityState = text.split(',').map(s => s.trim()).join(', ');
    ...
    try {
      ...
      // 1. UPDATE USER (Activating the Pass)
      ...
      await prisma.user.update({
        ...
        data: {
          ...
          // name: ctx.from.first_name || 'Member', // Ensuring 'name' is also present for your frontend gate
      ...
      const partners = await prisma.partner.findMany({
        ...
        where: {
          ...
          country: { equals: ctx.session.country, mode: 'insensitive' },

          cityState: { equals: normalizedCityState, mode: 'insensitive' }

        }

      });

      // 3. GENERATE RETURN BUTTONS

      let keyboard;

      if (partners.length === 0) {

        // No local partners? Give them the Lilburn Gym as a default premium option

        keyboard = Markup.inlineKeyboard([

          [Markup.button.url("🏋️ Enter Lilburn Fitness", `https://t.me/SmartSnailBot/app?startapp=gymId_${LILBURN_ID}`)]
        ...
        ctx.reply(
          ...
          `✅ SmartSnail Pass Registered!\n\nNo partners in ${normalizedCityState} yet, but you can explore our flagship facility:`,
        ...
        // Build buttons for all found partners

        const buttons = partners.map(p => [

          Markup.button.url(`🏢 Enter ${p.name}`, `https://t.me/SmartSnailBot/app?startapp=gymId_${p.id.toString()}`)

        ]);

        

        ctx.reply("🔥 Pass Activated! Your identity is verified.\nSelect a facility to enter:", Markup.inlineKeyboard(buttons));
      ...
      ctx.reply("⚠️ Something went wrong activating your pass. Please try /signup again.");
  ...
  for (const stake of fight.stakes) {
   ...
   if (fighter.ownerId) {
  const managerRecord = await tx.user.findUnique({ where: { id: fighter.ownerId } });

  const actualClawback = managerRecord.points >= managerCut ? managerCut : managerRecord.points;
...
bot.action(/set_sched_(.+)/, async (ctx) => {
  const adminId = process.env.NEXT_PUBLIC_ADMIN_TELEGRAM_ID;
  if (ctx.from.id.toString() !== adminId) {
    return ctx.answerCbQuery("❌ Only the Admin can do this.", { show_alert: true });
  }
  ...
  try {
    ...
    if (!sub) return ctx.reply("❌ Subscription not found.");

    // --- AUTO-ACTIVATE WALK-INS ---
    if (sub.planTitle.toLowerCase().includes("walk-in")) {
      const today = new Date();
      const currentDay = today.getDay(); // Get 0-6 for today

      await prisma.subscription.update({
        where: { id: subId },
        data: { 
          trainingDays: [currentDay], // Set only today
          startDate: today.toISOString(),
          status: 'ACTIVE' 
        }
      });

      await ctx.answerCbQuery("✅ Walk-in Activated!");
      return ctx.reply(`🥊 **Walk-in Active!**\n\nPlan: ${sub.planTitle}\nDay: Today (${['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][currentDay]})\n\nThe grid is now live for the user.`);
    }

    // --- MANUAL SETUP FOR SUBSCRIPTIONS ---
    ctx.session = ctx.session || {}; 
    ctx.session.step = 'awaiting_combat_days';
    ctx.session.targetSubId = subId;

    await ctx.reply("📅 **Subscription Detected**\n\nWhich days will they train? (0-6)\nExample: `1,3,5`", { parse_mode: 'Markdown' });
...
const SAGE_ADMIN_ID = process.env.NEXT_PUBLIC_ADMIN_TELEGRAM_ID;

  // ==== 1. SESSION INITIALIZATION ====
  ctx.session = ctx.session || {};

  // ==== 2. SAGE COMBAT SCHEDULE (PRIORITY) ====
  if (ctx.session?.step === 'awaiting_combat_days') {
    if (ctx.from.id.toString() !== SAGE_ADMIN_ID) {
      return ctx.reply('❌ Access Denied.');
    }

    const subId = ctx.session.targetSubId;
    ...
    try {
      ...
      // AUTO-DETECT WALK-IN: If title includes "Walk-In", just use today's day
      if (sub.planTitle.toLowerCase().includes("walk-in")) {
        days = [new Date().getDay()]; 
      } else {
        // Manual input for 3-month/6-month plans
        days = text.split(',')
      ...
      if (days.length === 0) {
        return ctx.reply('⚠️ Invalid format. Please send numbers (0-6) separated by commas.\nExample: 1,3,5');
      }
      ...
      await ctx.reply(`✅ **Schedule Locked!**\n\nPlan: ${sub.planTitle}\nDays: ${dayNames}\n\nThe grid is now LIVE in the app.`, { parse_mode: 'Markdown' });
      
      ctx.session.step = null;
      ctx.session.targetSubId = null;
      return; // Exit successfully
    } catch (error) {
      console.error('Combat Schedule Error:', error);
