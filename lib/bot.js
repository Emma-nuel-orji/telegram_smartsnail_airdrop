...
if (data.startsWith('select_fighter1:')) {
  const fighterId = parseInt(data.split(':')[1]);
  const fight = newFights[chatId];
  if (!fight) return ctx.answerCbQuery('❌ No active fight scheduling.');

  fight.fighter1Id = fighterId;
  fight.step = 'fighter2';

  // Fetch fighters for fighter 2 selection
  const fighters = await prisma.fighter.findMany({
    where: { id: { not: fighterId } },
  });

  if (!fighters.length) {
    delete newFights[chatId];
    return ctx.editMessageText('⚠️ Not enough fighters for Fighter 2 selection.');
  }

  const keyboard = fighters.map(f => [{ text: f.name, callback_data: `select_fighter2:${f.id}` }]);
  await ctx.editMessageText('👤 Select Fighter 2:', {
...
if (data.startsWith('select_fighter2:')) {
  ...
  const fighter1 = await prisma.fighter.findUnique({ where: { id: fight.fighter1Id } });
  const fighter2 = await prisma.fighter.findUnique({ where: { id: fight.fighter2Id } });

  fight.fighter1Name = fighter1?.name;
  fight.fighter2Name = fighter2?.name;

  await ctx.editMessageText(
    `✅ Fighters selected!\n\n${fight.fighter1Name} 🆚 ${fight.fighter2Name}\n\n📅 Now enter fight date/time (YYYY-MM-DD HH:mm):`
  );
  return;
}
