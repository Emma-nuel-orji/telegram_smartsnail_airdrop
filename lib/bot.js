...
// Reuse prisma and bot across requests
export const prisma =
  globalForBot.prisma || new PrismaClient();

if (!globalForBot.prisma) {
  globalForBot.prisma = prisma;
}
...
bot.on('photo', async (ctx) => {
  ...
  if (!ctx.session?.step || !ctx.session?.data) {
    ...
    return ctx.reply('❌ No ongoing fighter registration. Please start with /add_fighter.');
  }
