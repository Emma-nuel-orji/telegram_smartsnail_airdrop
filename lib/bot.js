...
if (data.startsWith('select_fighter1:')) {
  const fighterId = BigInt(data.split(':')[1]); // ✅ Use BigInt
  ...
  // Exclude fighter1
  const fighters = await prisma.fighter.findMany({
    where: { id: { not: fighter.id } }, // fighter.id is already BigInt
  });

  const keyboard = fighters.map(f => [
    { text: f.name, callback_data: `select_fighter2:${f.id.toString()}` } // ✅ convert to string for callback
  ]);
...
if (data.startsWith('select_fighter2:')) {
  const fighterId = BigInt(data.split(':')[1]); // ✅ BigInt
  const fighter = await prisma.fighter.findUnique({ where: { id: fighterId } });
  if (!fighter) return ctx.answerCbQuery('❌ Fighter not found.');
