...
bot.start(async (ctx) => {
    try {
        ...
        try {
            ...
            // Critical: Wait for response to ensure DB write is acknowledged
            await userResponse.text();

            if (userResponse.ok || userResponse.status === 409) {
                userCreatedOrFound = true;
                console.log(`User ${userId} confirmed in database`);
            } else {
        ...
        // Step 2: Process referral if payload exists
        let referralProcessed = false;
        const referrerId = startPayload?.trim();

        if (referrerId && referrerId !== '' && referrerId !== 'signup_smartsnail_pass') {
            try {
                // Validation: Only numbers and not self-referral
                if (/^\d+$/.test(referrerId) && referrerId !== userId.toString()) {
                    
                    // Delay to allow Database indexing to finish
                    await new Promise(resolve => setTimeout(resolve, 2000));
                    
                    console.log(`Processing referral from ${referrerId} to ${userId}`);
                ...
                // We don't alert the user here to keep the "Web3 Feel" smooth
            }
        }

        // Step 3: Handle SmartSnail Pass Registration Payload
        ...
        console.error('Global Error in start handler:', error);
        try {
            await ctx.reply('Welcome to SmartSnail! Click the button below to start farming.', {
                reply_markup: {
                    inline_keyboard: [[{ 
                        text: "🚀 Open App", 
