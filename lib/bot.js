...
bot.start(async (ctx) => {
    try {
        ...
        // Step 2: Process referral with retry logic
        ...
        if (startPayload && startPayload.trim() !== '') {
            ...
            if (!/^\d+$/.test(referrerId)) {
                ...
                const processReferralWithRetry = async (maxRetries = 3) => {
                    for (let attempt = 1; attempt <= maxRetries; attempt++) {
                        await new Promise(resolve => setTimeout(resolve, attempt * 1000));
                        console.log(`🔄 Referral attempt ${attempt} for user ${userId}`);
                        ...
                        try {
                            ...
                            console.log(`Attempt ${attempt} response:`, referralData);

                            if (referralResponse.status === 400 || referralResponse.status === 500) {
                                console.log(`⏳ User not ready yet (${referralResponse.status}), retrying...`);
                                continue;  // ✅ retry on 400 (user not found) and 500 (DB crash)
                              }

                            return { referralResponse, referralData };
                        } catch (fetchError) {
                ...
                try {
                    const { referralResponse, referralData } = await processReferralWithRetry();
                    ...
                    console.error('Referral error after retries:', error);
