...
bot.on('photo', async (ctx) => {
  ...
  if (!ctx.session?.step || !ctx.session?.data) {
    return ctx.reply('❌ No ongoing fighter registration. Please start with /register_fighter.');
  }
  ...
  if (ctx.session.step === FIGHTER_STEPS.IMAGE) {
    ...
    try {
      ...
      // Get file link with error handling
      let fileLink;
      try {
      ...
      // Success - store the URL and continue
      ...
      console.error('Error in image upload step:', error);
      ...
      if (loadingMsg) {
        try {
          ...
          console.log('Could not delete loading message after error:', deleteError.message);
        }
      }
      ...
      errorMessage += 'Type "skip" to continue without photo, or try again with a different image.';
