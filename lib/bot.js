...
bot.on("message:successful_payment", async (ctx) => {
  const payment = ctx.message.successful_payment;
  const payload = JSON.parse(payment.invoice_payload);
  const { fightId, fighterId, stakeAmount, telegramId } = payload;
  const shellAmount = BigInt(stakeAmount);

  try {
    await prisma.$transaction(async (tx) => {
      const fighter = await tx.fighter.findUnique({ where: { id: fighterId } });

      // INSTANT COMMISSIONS (50% Manager / 30% Fighter)
    ...
    console.log(`✅ Stars Payment Finalized for ${telegramId}`);
  } catch (error) {
    console.error("❌ Stars Payment Finalization Error:", error);
  }
});
...
async function resolveFight(fightId, status, winnerId) {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (status === "COMPLETED" && winnerId) {
      ...
      const scaledMultiplier = (totalPayoutPool * 100n) / winnerSideDenominator;
        ...
        if (rewardNft) {
          ...
          if (recipientId) {
            ...
            mintedNftData = {
              ...
              collection: winnerFighter.collection?.name || "Polycombat"
            };
          }
        }
      }
...
bot.on('callback_query', async (ctx) => {
  ...
  if (data.startsWith('check_f_')) {
    ...
    await ctx.editMessageText(`👤 *${fighter.name}*\nStatus: ${status}`, {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [
  ...
  if (data.startsWith('set_team_')) {
    ...
    await ctx.editMessageText(`✅ Success: ${teamName} assigned.`);
  }
});
