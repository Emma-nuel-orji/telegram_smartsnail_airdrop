        ...
        if (startPayload && startPayload.trim() !== '') {
            try {
                ...
                if (!/^\d+$/.test(referrerId)) {
                    ...
                    console.log('Referral API Response Data:', referralData);
...
async function resolveFight(fightId, status, winnerId) {
    ...
    if (winnerId !== undefined) {
        data.winnerId = winnerId; // winnerId should be a string (ObjectId)
    }
    
    try {
        return await prisma.fight.update({
            where: { id: fightId }, // fightId should be string
            data,
        });
    } catch (error) {
