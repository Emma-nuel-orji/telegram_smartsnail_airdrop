...
bot.on("message:successful_payment", async (ctx) => {
  ...
  try {
    ...
    if (payloadData.type === "ticket_purchase") {
      const { ticketId, telegramId, userName, ticketType, quantity, totalCost, pricePerTicket } = payloadData;
      
      // Check for duplicates
      const exists = await prisma.ticket.findUnique({ where: { ticketId } });
      if (exists) {
      ...
      await ctx.reply(
        ...
        `💰 ${totalCost.toLocaleString()} Stars\n\n` +
        `Pending admin approval. You'll be notified!`,
        { parse_mode: "Markdown" }
      );
    }
  } catch (error) {
...
if (data.startsWith('select_fighter1:')) {
  const fighterId = data.split(':')[1]; // ✅ String, matches schema
  const fighter = await prisma.fighter.findUnique({ where: { id: fighterId } });
  if (!fighter) return ctx.answerCbQuery('❌ Fighter not found.');
  ...
  // Exclude fighter1
  const fighters = await prisma.fighter.findMany({
    where: { id: { not: fighter.id } }, // ✅ fighter.id is String
  });

  const keyboard = fighters.map(f => [
    { text: f.name, callback_data: `select_fighter2:${f.id}` } // ✅ keep as string
  ]);
...
if (data.startsWith('select_fighter2:')) {
  const fighterId = data.split(':')[1]; // ✅ String
  const fighter = await prisma.fighter.findUnique({ where: { id: fighterId } });
  if (!fighter) return ctx.answerCbQuery('❌ Fighter not found.');
  ...
  return ctx.editMessageText(
    `📅 Now enter the fight date & time in format: YYYY-MM-DD HH:mm\n\nExample: 2025-09-30 18:30`
  );
}
...
if (data.startsWith('resolve_win:')) {
  ...
  await resolveFight(fightIdStr, 'COMPLETED', winnerIdStr); // all strings
