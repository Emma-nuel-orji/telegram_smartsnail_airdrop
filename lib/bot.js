...
bot.command('add_menu_item', async (ctx) => {
    ...
    if (!(await isMenuManager(ctx, adminPartner.id))) {
...
bot.command('list_menu', async (ctx) => {
    ...
    try {
        ...
        if (adminPartner && (await isMenuManager(ctx, adminPartner.id))) {
            // Admin can see their partner's menu
        ...
        } else if (await isSuperAdmin(ctx)) {
            ...
            // Regular users can see all menu items (public view)
            menuItems = await prisma.service.findMany({
                where: { type: 'ONE_TIME' },
                include: { partner: true },
                orderBy: { createdAt: 'desc' }
...
bot.command('edit_menu_item', async (ctx) => {
    ...
    try {
        ...
        if (menuItems.length === 0) {
            ctx.reply('📋 No menu items to edit for your partner.');
            return;
        }
        ...
        ctx.reply(`📝 Select a menu item to edit for ${adminPartner.name}:`, {
...
bot.command('delete_menu_item', async (ctx) => {
    ...
    try {
        ...
        if (menuItems.length === 0) {
            ctx.reply('📋 No menu items to delete for your partner.');
            return;
        }
        ...
        ctx.reply(`🗑️ Select a menu item to delete for ${adminPartner.name}:`, {
            reply_markup: { inline_keyboard: buttons }
        });
    } catch (error) {
...
bot.command('add_gym_service', async (ctx) => {
    ...
    if (!adminPartner || adminPartner.type !== 'GYM') {
        ctx.reply('❌ You must be a gym administrator to use this command.');
        return;
    }

    if (!(await isGymAdmin(ctx, adminPartner.id))) {
        ctx.reply('❌ You do not have permission to manage services for this gym.');
        return;
    }
