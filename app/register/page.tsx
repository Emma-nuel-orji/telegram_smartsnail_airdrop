...
import { Camera, User, Ruler, Weight, Trophy, Hash, Check, ChevronLeft, AlertCircle, Zap, ShieldCheck, Download, Share2 } from 'lucide-react';
...
export default function FighterRegistration() {
    ...
    try {
      ...
      if (webApp?.shareToStory) {
        webApp.shareToStory(publicUrl, {
          text: `🥊 I just signed my contract! Support me in the SCL!`,
  ...
  const downloadCard = async () => {
    ...
    link.download = `SCL-CARD-${formData.name}.jpeg`;
    link.href = dataUrl;
    link.click();
  };
  ...
  if (success) return (
    <div className="min-h-screen bg-black text-white p-6 flex flex-col items-center justify-center space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-black uppercase italic tracking-tighter">Verified</h2>
      </div>
      <div ref={cardRef} className="relative w-72 h-[440px] bg-[#050505] border border-red-600/30 rounded-2xl overflow-hidden shadow-2xl">
        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_50%_0%,_#dc2626_0%,_transparent_70%)]" />
        <div className="absolute top-4 right-4 z-20">
          {formData.team ? <div className="bg-white text-black text-[9px] font-black px-2 py-1 uppercase italic rounded-sm border-l-4 border-red-600">{formData.team}</div> : <div className="text-[7px] text-zinc-500 font-bold uppercase tracking-widest bg-black/50 px-2 py-1 rounded-full border border-white/10 backdrop-blur-md">Pending...</div>}
        </div>
        <div className="relative z-10 p-4 flex justify-between items-center">
          <div className="bg-red-600 text-white text-[8px] font-black px-2 py-0.5 italic uppercase skew-x-[-10deg]">SCL CERTIFIED</div>
        ...
        <div className="p-5 text-center relative z-10">
          ...
          <p className="text-red-600 text-[10px] font-black uppercase tracking-[0.3em] mb-6">{formData.weightClass}</p>
      ...
      <div className="flex flex-col gap-3 w-full max-w-xs">
        <button onClick={shareToStory} className="w-full bg-red-600 text-white p-4 rounded-xl font-black uppercase text-xs flex items-center justify-center gap-2 shadow-lg"><Share2 className="w-4 h-4" /> Share to Story</button>
        <button onClick={downloadCard} className="w-full bg-white text-black p-4 rounded-xl font-black uppercase text-xs flex items-center justify-center gap-2"><Download className="w-4 h-4" /> Save</button>
      </div>
    </div>
  );
  ...
  // --- RENDER FORM ---
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-zinc-100 p-4">
      <div className="relative z-10 max-w-xl mx-auto pt-10 pb-20">
        <div className="mb-10 flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-black italic tracking-tighter uppercase leading-none">Registration</h1>
            <p className="text-red-600 text-[10px] font-bold uppercase tracking-[0.4em] mt-2 flex items-center gap-2"><Zap className="w-3 h-3 fill-current" /> Sage Combat League</p>
          </div>
        ...
        {formData.name && step < steps.length && (
          <div className="mb-8 p-4 bg-zinc-900 border-l-4 border-red-600 rounded-r-xl animate-in slide-in-from-right">
            <div className="flex gap-4 items-center">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-zinc-500 uppercase">{formData.weightClass || 'Classifying...'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {step < steps.length ? (
          <div className="space-y-8">
            ...
            {currentStepData.type === 'select' ? (
              <div className="grid grid-cols-2 gap-3 mt-4">
                {currentStepData.options?.map((opt) => (
                  <button key={opt} onClick={() => setFormData(p => ({...p, [currentStepData.field]: opt}))} className={`py-4 px-2 text-xs font-black uppercase tracking-widest border transition-all ${formData[currentStepData.field] === opt ? 'bg-red-600 border-red-600 text-white' : 'bg-zinc-900 border-white/5 text-zinc-500'}`}>{opt}</button>
                ))}
              </div>
            ) : currentStepData.type === 'file' ? (
              <label className="mt-4 flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-zinc-800 rounded-3xl bg-zinc-900/50 cursor-pointer overflow-hidden group">
                <input type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
                {formData.photoPreview ? <img src={formData.photoPreview} className="h-full w-full object-cover" /> : <Camera className="w-12 h-12 text-zinc-700 group-hover:text-red-600 transition-colors" />}
              </label>
            ) : (
              <input autoFocus type={currentStepData.type || 'text'} name={currentStepData.field} value={(formData[currentStepData.field] as string) || ''} onChange={handleInputChange} placeholder={currentStepData.placeholder} className="w-full bg-zinc-900 border-b-2 border-zinc-800 p-6 text-2xl font-black italic uppercase focus:border-red-600 outline-none transition-all" />
            )}
            ...
            <div className="flex gap-4 pt-10">
              {step > 0 && <button onClick={() => setStep(s => s - 1)} className="p-5 bg-zinc-900 text-zinc-500 rounded-2xl"><ChevronLeft className="w-6 h-6" /></button>}
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom">
            <div className="text-center"><h2 className="text-3xl font-black italic uppercase mb-2">Sanctioning Fee</h2></div>
            <div className="grid gap-4">
              <button onClick={() => setPaymentMethod('stars')} className={`flex items-center justify-between p-6 rounded-3xl border-2 transition-all ${paymentMethod === 'stars' ? 'border-yellow-500 bg-yellow-500/5' : 'border-zinc-800 bg-zinc-900/50'}`}>
                <div className="flex items-center gap-4"><div className="text-2xl">⭐</div><div className="text-left leading-none"><span className="block font-black italic uppercase text-xl">{REGISTRATION_COST_STARS} Stars</span><span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Official Entry</span></div></div>
                {paymentMethod === 'stars' && <Check className="w-6 h-6 text-yellow-500" />}
              </button>
            </div>
            <button onClick={handleSubmit} disabled={isSubmitting || !paymentMethod} className="w-full bg-red-600 text-white font-black uppercase italic text-xl p-5 disabled:grayscale shadow-lg active:scale-95 transition-all">
