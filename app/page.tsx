...
export default function Home() {
  ...
  useEffect(() => {
    const initializeTelegram = async () => {
      try {
        ...
        setUser(data);
        // Show welcome popup if user hasn't claimed welcome bonus
  ...
  const handleIncreasePoints = async (e: React.MouseEvent) => {
    ...
    const pointsToAdd = user.tappingRate;
    ...
    try {
      ...
      if (data.success) {
        setNotification(`+${pointsToAdd} shells!`);
  ...
  const handleSpeedAndAnimation = (e: React.MouseEvent) => {
    ...
    // Calculate click position relative to the target element
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setUser(prev => prev ? { ...prev, points: data.points, hasClaimedWelcome: true } : null);
  ...
  return (
    <div className="bg-gradient-main min-h-screen px-4 flex flex-col items-center text-white font-medium">
      ...
      <div className="w-full z-10 min-h-screen flex flex-col items-center">
        ...
        <div className="flex-grow flex items-center justify-center relative" onClick={handleIncreasePoints}>
          <div className="relative mt-4">
            <video src="/images/snails.mp4" autoPlay muted loop playsInline className="rounded-lg shadow-lg" />
            {clicks.map((click) => (
              <div
                key={click.id}
      ...
      <style jsx>{`
        ...
        .bg-gradient-overlay {
          background: linear-gradient(180deg, rgba(26,26,26,0) 0%, #000000 100%);
        }
